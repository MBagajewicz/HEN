...
# Instruction for Pyomo to create the model "model", # Let us change it to "Lower_Bound"
model = ConcreteModel()

# --------------------------
# Sets
# Define the sets of Hot and cold streams and the stages.
...
model.mhph = Var(model.SI, model.SP, model.SK, within=NonNegativeReals, bounds=(0, 400))

# Heat-pump fluid mass flowrate in the match between cold stream j and heat pump p (condenser side).
model.mhpc = Var(model.SJ, model.SP, model.SK, within=NonNegativeReals, bounds=(0, 400))
...
def constrs_rule39(model,p):
    return model.w[p] == float(heatpump2.dhp[p]) * model.mhp[p]
# --------------------------
# Attach constraints
# --------------------------
...
#model.constrs39 = Constraint(model.SP, rule=constrs_rule39)
#model.constrs40 = Constraint(model.SP, rule=constrs_rule40)
...
if (res.solver.termination_condition == TerminationCondition.optimal or
    ...
    # ---- Print heat-load and power variables (non-zero only) ----
    print("\nExchanger duties q(i,j,k):")
    for i in model.SI:
        for j in model.SJ:
            for k in model.ST:
                if value(model.q[i, j, k]) > 1e-6:
                    print(f"q[{i},{j},{k}] = {value(model.q[i, j, k]):.4f}")

    print("\nHeater duties qh(j):")
    for j in model.SJ:
        if value(model.qh[j]) > 1e-6:
            print(f"qh[{j}] = {value(model.qh[j]):.4f}")

    print("\nCooler duties qc(i):")
    for i in model.SI:
        if value(model.qc[i]) > 1e-6:
            print(f"qc[{i}] = {value(model.qc[i]):.4f}")

    print("\nEvaporator duties qhph(i,p,k):")
    for i in model.SI:
        for p in model.SP:
            for k in model.ST:
                if value(model.qhph[i, p, k]) > 1e-6:
                    print(f"qhph[{i},{p},{k}] = {value(model.qhph[i, p, k]):.4f}")

    print("\nCondenser duties qhpc(j,p,k):")
    for j in model.SJ:
        for p in model.SP:
            for k in model.ST:
                if value(model.qhpc[j, p, k]) > 1e-6:
                    print(f"qhpc[{j},{p},{k}] = {value(model.qhpc[j, p, k]):.4f}")

    print("\nCompressor power w(p):")
    for p in model.SP:
        if value(model.w[p]) > 1e-6:
            print(f"w[{p}] = {value(model.w[p]):.4f}")
else:
